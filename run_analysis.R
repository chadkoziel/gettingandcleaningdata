setwd("C:/Users/chad.koziel/OneDrive/Documents/Coursera R/Getting and Cleaning Data/UCI HAR Dataset")

library(dplyr)

# read in feature & activity data sets
features <- read.table("features.txt",header=FALSE)
activities <- read.table("activity_labels.txt",header=FALSE,col.names = c("V1","Activity"))
...
features_filt$V1 <- paste("V", features_filt$V1,sep = "", collapse = NULL)

# read in desired features from x datasets, combine, add descriptive column names
x_train <- read.table("train/X_train.txt",header=FALSE)[features_filt$V1]
x_test <- read.table("test/X_test.txt",header=FALSE)[features_filt$V1]
...
rm("features","features_filt",
     "activities",
     "x_train","x_test","x",
     "y_train","y_test","y",
     "subjects_train","subjects_test","subjects")
